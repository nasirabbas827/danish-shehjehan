...
from .models import Election, VoteBlocks, Candidate
...
def vote(request, election_id, candidate_id):
    ...
    # Add the block to the blockchain and get the blockchain code
    blockchain_code = blockchain.add_block_to_database(user, candidate)
...
def election_winners(request):
    ...
    for election in completed_elections:
        has_voted = VoteBlocks.objects.filter(user=user, candidate__election=election).exists()
        ...
        if has_voted:
            ...
            candidate_votes = Counter(
                VoteBlocks.objects.filter(candidate__election=election).values_list('candidate_id', flat=True)
            )
